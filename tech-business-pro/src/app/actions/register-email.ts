...
import { SeekerWelcomeEmail } from '@/components/email/seeker-welcome-email';

const resend = new Resend(process.env.RESEND_API_KEY);

export async function sendSeekerWelcomeEmail(
  email: string,
  name: string,
  username: string,
) {
  try {
    const emailHtml = await renderAsync(
      React.createElement(SeekerWelcomeEmail, { name, username }),
    );
